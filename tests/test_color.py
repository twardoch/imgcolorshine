...
def test_batch_oklab_to_rgb(engine: OKLCHEngine, oklab_shape: tuple):
    ...
    assert np.all(rgb_image >= -0.001) and np.all(rgb_image <= 1.001) # Check if values are roughly in [0,1] after gamut mapping
...
def test_oklab_to_rgb_single(engine: OKLCHEngine):
    ...
    assert np.all(rgb_np >= 0) and np.all(rgb_np <= 1)
