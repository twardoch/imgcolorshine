...
class TestTransformPixelsGPU:
    ...
    def test_transform_pixels_gpu_basic(self):
        ...
        attractors_lch = trans_gpu.batch_oklab_to_oklch_gpu(
            attractors_lab.reshape(1, 1, 3), xp=np
        ).reshape(-1, 3)
        ...
        result = trans_gpu.transform_pixels_gpu(
            oklab, oklch, attractors_lab, attractors_lch, 
            tolerances, strengths, flags, xp=np
        )
    ...
    def test_transform_pixels_gpu_channel_control(self):
        ...
        result_l = trans_gpu.transform_pixels_gpu(
            oklab, oklch, attractors_lab, attractors_lch,
            tolerances, strengths, flags_l, xp=np
        )
        
        result_c = trans_gpu.transform_pixels_gpu(
            oklab, oklch, attractors_lab, attractors_lch,
            tolerances, strengths, flags_c, xp=np
        )
        
        result_h = trans_gpu.transform_pixels_gpu(
            oklab, oklch, attractors_lab, attractors_lch,
            tolerances, strengths, flags_h, xp=np
        )
...
class TestProcessImageGPU:
    ...
    def test_process_image_gpu_full_pipeline(self):
        ...
        with (
            patch('imgcolorshine.trans_gpu.check_gpu_memory_available', 
                  return_value=(False, 100, 1000))
        ):
            result = trans_gpu.process_image_gpu(
                rgb_image, attractors_lab, tolerances, strengths,
                enable_luminance=True, enable_saturation=True, enable_hue=True
            )
    ...
    def test_process_image_gpu_with_mock_success(self):
        ...
        with (
            ...
            result = trans_gpu.process_image_gpu(
                rgb_image, attractors_lab, tolerances, strengths,
                enable_luminance=True, enable_saturation=False, enable_hue=True
            )
...
class TestMultipleAttractors:
    ...
    def test_transform_pixels_gpu_multiple_attractors(self):
        ...
        attractors_lch = trans_gpu.batch_oklab_to_oklch_gpu(
            attractors_lab.reshape(3, 1, 3), xp=np
        ).reshape(-1, 3)
