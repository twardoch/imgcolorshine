...
def test_compute_gradient_magnitude():
    ...
    assert np.all(grad_edge[0, :] == 0) and np.all(grad_edge[-1, :] == 0)
    assert np.all(grad_edge[:, 0] == 0) and np.all(grad_edge[:, -1] == 0)
...
def test_compute_difference_mask(mock_compute_mask, mock_batch_srgb_to_oklab, processor_default: HierarchicalProcessor):
    ...
    np.testing.assert_array_almost_equal(mock_batch_srgb_to_oklab.call_args_list[0][0][0], fine_level_rgb.astype(np.float32)/255.0)
    np.testing.assert_array_almost_equal(mock_batch_srgb_to_oklab.call_args_list[1][0][0], coarse_upsampled_rgb.astype(np.float32)/255.0)
...
def test_detect_gradient_regions(mock_dilate, mock_getse, mock_compute_grad, mock_cvtcolor, processor_default: HierarchicalProcessor):
...
@mock.patch('cv2.getStructuringElement')
@mock.patch('cv2.dilate')
def test_detect_gradient_regions_no_gradient(mock_dilate, mock_getse, mock_compute_grad, mock_cvtcolor, processor_default: HierarchicalProcessor):
    image_rgb = np.random.randint(0, 256, (64, 64, 3), dtype=np.uint8)
    gradient_threshold = 0.1
...
def mock_transform_func(image, attractors, tolerances, strengths, channels):
    ...
    return (255 - (image * 255).astype(np.uint8)).astype(np.uint8) if image.max() <=1 else (255 - image).astype(np.uint8)


@mock.patch('cv2.resize') # Removed problematic side_effect
def test_process_hierarchical_small_image_no_pyramid(mock_cv_resize, processor_default: HierarchicalProcessor, sample_image_rgb_uint8: np.ndarray):
    ...
    with mock.patch.object(processor_default, 'build_pyramid', return_value=[PyramidLevel(image=small_img, scale=1.0, shape=small_img.shape[:2], level=0)]) as mock_build_pyr:
...
@mock.patch('cv2.resize') # Removed problematic side_effect
def test_process_hierarchical_full_run(mock_cv_resize, processor_default: HierarchicalProcessor, sample_image_rgb_uint8: np.ndarray):
    # Mock sub-functions to control their behavior and verify calls
    with mock.patch.object(processor_default, 'compute_difference_mask') as mock_comp_diff, \
        ...
        result = processor_default.process_hierarchical(sample_image_rgb_uint8, mock_transform_func, att, tol, stren, chan)
        ...
        upsampled_from_coarsest = cv2.resize(transformed_coarsest, (sample_image_rgb_uint8.shape[1], sample_image_rgb_uint8.shape[0]), interpolation=cv2.INTER_LINEAR)
...
def test_process_hierarchical_no_adaptive_subdivision(mock_cv_resize, processor_no_adaptive: HierarchicalProcessor, sample_image_rgb_uint8: np.ndarray):
    # This test uses processor_no_adaptive fixture (use_adaptive_subdivision=False)
    with mock.patch.object(processor_no_adaptive, 'compute_difference_mask') as mock_comp_diff, \
        ...
        result = processor_no_adaptive.process_hierarchical(sample_image_rgb_uint8, mock_transform_func, att, tol, stren, chan)
...
def test_process_hierarchical_no_refinement_needed(processor_default: HierarchicalProcessor, sample_image_rgb_uint8: np.ndarray):
    ...
    with mock.patch.object(processor_default, 'compute_difference_mask', return_value=np.zeros(sample_image_rgb_uint8.shape[:2], dtype=bool)) as mock_comp_diff, \
         mock.patch.object(processor_default, 'detect_gradient_regions', return_value=np.zeros(sample_image_rgb_uint8.shape[:2], dtype=bool)) as mock_detect_grad:
        ...
        coarsest_img = cv2.pyrDown(sample_image_rgb_uint8)
        transformed_coarsest = mock_transform_func(coarsest_img, att, tol, stren, chan)
        expected_result = cv2.resize(transformed_coarsest, (sample_image_rgb_uint8.shape[1], sample_image_rgb_uint8.shape[0]), interpolation=cv2.INTER_LINEAR) # As per code
...
def test_process_hierarchical_tiled_small_image_no_tiling(processor_default: HierarchicalProcessor, sample_image_rgb_uint8: np.ndarray):
...
def test_process_hierarchical_tiled_large_image(processor_default: HierarchicalProcessor):
    ...
    with mock.patch.object(processor_default, 'process_hierarchical', side_effect=simple_tile_processor) as mock_proc_hier:
        result = processor_default.process_hierarchical_tiled(large_img, mock_transform_func, [], [], [], [], tile_size=tile_size)
